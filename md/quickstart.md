...
<h2 id="third-party-api">
  使用第三方 API（DeepSeek / OpenAI 兼容端点）
</h2>

Claude Code 除了使用官方 Anthropic API 外，还支持通过 **Anthropic 兼容端点**或 **OpenAI 兼容端点**接入第三方模型服务（如 DeepSeek、硅基流动、OpenRouter 等）。这样可以绕开 claude.ai 的网络限制，也可以使用自己已有的 API Key 降低成本。

> **原理**：Claude Code 本身是一个 Agent 框架（Harness），模型层是可替换的。设置环境变量或启动参数后，它会将请求转发到你指定的端点，而不是默认的 Anthropic API。

<h3 id="env-var-config">
  方式一：环境变量配置（推荐）
</h3>

在启动 `claude` 命令前，先设置以下环境变量：

* `ANTHROPIC_BASE_URL`：第三方 API 的地址（如 DeepSeek 为 `https://api.deepseek.com/v1`）
* `ANTHROPIC_API_KEY`：你的 API Key
* `ANTHROPIC_MODEL`：要使用的模型名称（如 `deepseek-v4-flash`）
...
<h3 id="settings-json-config">
  方式二：settings.json 配置
</h3>

Claude Code 的全局配置文件位于 `~/.claude/settings.json`（Windows 为 `C:\Users\你的用户名\.claude\settings.json`）。在 `env` 字段中配置：
...
<h3 id="cli-flag-config">
  方式三：命令行参数
</h3>

也可以在启动时直接通过参数指定模型提供商：

```bash
claude --model deepseek-v4-flash
```

如果你的第三方服务使用 OpenAI 兼容接口，部分版本还支持通过 `--provider openai` 配合 `--api-key` 参数直接指定：

```bash
claude --provider openai --api-key sk-你的Key --model deepseek-v4-flash --api-base https://api.deepseek.com/v1
```

<h3 id="common-providers">
  常见第三方服务参考
</h3>

| 服务商 | Base URL | 说明 |
|:--|:--|:--|
| DeepSeek | `https://api.deepseek.com/v1` | 国产开源模型，价格低，中文友好 |
| 硅基流动 | `https://api.siliconflow.cn/v1` | 聚合多家开源模型 |
| OpenRouter | `https://openrouter.ai/api/v1` | 聚合 Claude / GPT / Gemini 等 |
| 通义千问 | `https://dashscope.aliyuncs.com/compatible-mode/v1` | 阿里云百炼，OpenAI 兼容 |

> **提示**：使用第三方 API 时不需要执行 `/login` 登录 Anthropic 账户。如果启动时仍然弹出登录界面，说明环境变量没有正确加载，请检查上述配置是否生效。

<h3 id="verify-config">
  验证配置是否生效
</h3>

启动 `claude` 后输入任意问题，观察响应是否来自你配置的模型。也可以在终端执行：

```bash
claude --version
```

确认安装版本后，直接发起一次对话即可验证 API 连通性。如果报 401，说明 API Key 错误；如果报 404，说明 Base URL 或模型名不对。
